...
function checkWin(marked: boolean[][]) {
        ...
        if (room && room.currentTurn === socket.id && room.gameStarted) {
          if (!room.markedNumbers.includes(number)) {
            ...
            io.to(roomId).emit('number-marked', { 
              number, 
              markedBy: room.players.find((p: any) => p.id === socket.id)?.name,
        ...
        if (room && room.gameStarted) {
          ...
          if (player && !player.hasWon) {
            ...
            if (isValid) {
              ...
              const winners = room.players.filter((p: any) => p.hasWon);
              if (winners.length > 1) {
